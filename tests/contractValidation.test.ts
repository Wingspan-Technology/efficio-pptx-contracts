...
describe("validateAllContracts — structured report over an injected contracts dir", () => {
  ...
  it("rejects migration gaps and unknown fields or operations", async () => {
    ...
    report = await validateAllContracts({ contractsDir: dir });
    expect(report.issues.some((issue) => /must be derive_text_character_limits, migrate_text_capacity, rename_tag, or set_tag_if_missing/.test(issue.message)))
      .toBe(true);
  });
  ...
  it("accepts an empty operations array but still requires the field", async () => {
    ...
    expect(
      report.issues.some((issue) =>
        /must be derive_text_character_limits, migrate_text_capacity, rename_tag, or set_tag_if_missing/.test(issue.message),
      ),
    ).toBe(true);
  });
