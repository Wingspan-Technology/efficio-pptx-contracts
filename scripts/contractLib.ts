...
// ─── Tag Entity → legacy compatibility output ──────────────────────────────────
...
// Effective merged native Tag Entity map (common tags + component tags), ordered
// common-first then component-only. Only efficio_component_type may be shared
// (the component refines it by adding its single-value enum). Used to build the
// native component metadata; the compatibility derivation stays in mergeTagSchema.
...
export function mergeTagSchema(
  ...
  // Only efficio_component_type may be shared; the component refines it by adding
  // its single-value enum. Any other shared tag is a duplicate-definition conflict.
  ...
  // enums: common-authored enums first, then component-authored enums (this is
  // where efficio_component_type's enum lands, since common does not declare it).
  ...
  // types: common entities (merged, so efficio_component_type reads as enum) first,
  // then component-only entities.
  ...
  // json_schemas: embedded JSON Schemas for object/array (json_object/json_array)
  // tags, keyed by tag name. Same common-first then component-only ordering.
  const jsonSchemas: JsonObject = {};
  for (const key of commonKeys) {
    const schema = (mergedCommon[key] as JsonObject).schema;
